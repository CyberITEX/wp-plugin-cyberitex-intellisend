...
with sync_playwright() as runtime:
    ...
    def fixture(body, script, name):
        ...
        page.add_style_tag(path=str(ROOT / 'admin/css/theme.css'))
    ...
    check('prefers-color-scheme:dark' in frame.get_attribute('srcdoc'), 'Report preview follows the system colour mode')
    ...
    page.set_content('<body class="intellisend_page_intellisend-reports"><div class="intellisend-admin"><h1>Theme</h1><div class="intellisend-card"><h2>Card</h2><label for="theme-input">Input</label><input id="theme-input" value="Example"></div></div><div class="intellisend-modal-content">Dialog</div></body>')
    page.add_style_tag(path=str(ROOT / 'admin/css/theme.css'))
    light = page.locator('.intellisend-card').evaluate('(element) => { const style = getComputedStyle(element); return [style.backgroundColor, style.color]; }')
    page.emulate_media(color_scheme='dark')
    dark = page.locator('.intellisend-card').evaluate('(element) => { const style = getComputedStyle(element); return [style.backgroundColor, style.color]; }')
    dark_input = page.locator('#theme-input').evaluate('(element) => { const style = getComputedStyle(element); return [style.backgroundColor, style.color, style.colorScheme]; }')
    dark_modal = page.locator('.intellisend-modal-content').evaluate('(element) => { const style = getComputedStyle(element); return [style.backgroundColor, style.color]; }')
    check(light == ['rgb(255, 255, 255)', 'rgb(30, 30, 30)'], 'Light theme keeps the established card palette')
    check(dark == ['rgb(31, 41, 55)', 'rgb(248, 250, 252)'], 'Dark theme applies the shared surface and text palette')
    check(dark_input[0] == 'rgb(17, 24, 39)' and dark_input[1] == 'rgb(248, 250, 252)' and 'dark' in dark_input[2], 'Dark theme styles form controls and native colour scheme')
    check(dark_modal == ['rgb(31, 41, 55)', 'rgb(248, 250, 252)'], 'Dark theme reaches dialogs rendered outside the page wrapper')
    browser.close()
