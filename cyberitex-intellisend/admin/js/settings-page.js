...
(function ($) {
  ...
  const IntelliSendSettings = {
    ...
    setupLogsRetentionDropdown: function () {
      ...
      const $select = $(
        '<select id="logs-retention-select" class="logs-retention-select"></select>'
      );
    ...
    setupAutoSave: function () {
      ...
      $("#default-provider").on("change", function () {
        ...
        self.autoSaveSetting(
          "defaultProviderName",
          newValue,
          $field,
          "Default SMTP provider updated"
        );
      });
        ...
        if (newValue === "" || self.isValidEmail(newValue)) {
          testRecipientTimeout = setTimeout(function () {
            self.autoSaveSetting(
              ...
              newValue,
              $field,
              "Test recipient updated"
            );
          }, 1000); // 1 second delay
        }
      });

      // Auto-save debug toggle when changed
      $("#debug-enabled").on("change", function () {
        const $toggle = $(this);
        const $label = $toggle
          .closest(".toggle-switch-container")
          .find(".toggle-label");
        ...
        // Add visual feedback to the toggle container
        const $container = $toggle.closest(".toggle-switch-container");
        $container.addClass("auto-saving");

        self.autoSaveDebugSetting(
          "debug_enabled",
          newValue,
          $container,
          isEnabled
    ...
    /**
     * Auto-save debug setting with special handling
     */
    autoSaveDebugSetting: function (settingName, value, $container, isEnabled) {
      const self = this;
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            // Remove saving state and add success feedback
            $container.removeClass("auto-saving").addClass("auto-saved");

            // Show toast notification with current state
            const message = isEnabled
              ? "Debug mode enabled"
              : "Debug mode disabled";
            IntelliSendToast.success(message, {
              duration: 2000,
            });

            // Remove success class after animation
            setTimeout(function () {
              $container.removeClass("auto-saved");
            }, 2000);
          } else {
            // Handle error
            $container.removeClass("auto-saving").addClass("auto-save-error");

            // Revert toggle state on error
            const $toggle = $container.find("#debug-enabled");
            $toggle.prop("checked", !isEnabled);

            // Revert label text
            const $label = $container.find(".toggle-label");
            $label.text(!isEnabled ? "Enabled" : "Disabled");

            IntelliSendToast.error(
              "Failed to save debug setting: " +
                (response.data.message || "Unknown error")
            );

            setTimeout(function () {
              $container.removeClass("auto-save-error");
            }, 3000);
          }
        },
        error: function () {
          ...
          IntelliSendToast.error(
            "Network error occurred while saving debug setting"
          );
    ...
    handleFormSubmit: function () {
      ...
      $form.on("submit", function (e) {
        ...
        if (apiKey.trim() !== "") {
          ...
          // Show error message
          IntelliSendToast.error(
            "API key is required when an endpoint is specified"
          );
          self.resetButtonLoading($submitButton, originalButtonText);
          $form.find("button").prop("disabled", false);
        } else {
    ...
    validateApiKey: function (apiKey, endpoint, callback) {
      ...
      $.ajax({
        ...
        error: function (xhr, status, error) {
          // Network error
          IntelliSendToast.error(
            "A network error occurred while validating the API key. Please try again."
          );
          callback(false);
        },
      });
          ...
          if (response.success) {
            ...
            const errorMessage =
              response.data && response.data.message
                ...
                : "An error occurred while saving anti-spam settings.";
          ...
          success: function (response) {
            if (response.success) {
              ...
              const errorMessage =
                response.data && response.data.message
                  ? response.data.message
                  : "An error occurred during spam detection.";
              IntelliSendToast.error(errorMessage);
            }
          },
          error: function () {
    ...
    setupFormAnimations: function () {
      const $fields = $(
        ".intellisend-settings-field input, .intellisend-settings-field select, .intellisend-settings-field textarea"
      );
      ...
      $fields
        .on("focus", function () {
          $(this)
            .closest(".intellisend-settings-field")
    ...
    setupSectionToggle: function () {
      ...
      $(".intellisend-settings-section-title").append(
        '<span class="section-toggle-indicator dashicons dashicons-arrow-up-alt2"></span>'
      );
    ...
    setupPageLoadEffects: function () {
      ...
      $(
        '<div id="page-loading-overlay"><div class="loading-spinner-large"></div></div>'
      ).appendTo("body");
    ...
    resetButtonLoading: function ($button, originalText) {
      $button
        .html(originalText || $button.data("original-text"))
        .prop("disabled", false);
    },
